...
(
    ...
    `type`        tinyint      NOT NULL DEFAULT 1 COMMENT '类型(in:length:1)',
...
(
    ...
    `administrator` tinyint      NOT NULL DEFAULT 0 COMMENT '管理员(in:0,1)【0:否,1:是】',
    UNIQUE KEY `idx_user_group_administrator` (`user_id`, `role_id`, `group_id`, `administrator`) USING BTREE
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
...
(
    ...
    `where`   varchar(255) NOT NULL DEFAULT '' COMMENT '条件(noRequire)',
    `value`   varchar(255) NOT NULL DEFAULT '' COMMENT '值(noRequire)',
    PRIMARY KEY (`id`) USING BTREE,
...
(
    ...
    `menu_id`     int unsigned NOT NULL DEFAULT 0 COMMENT '父级菜单ID(noRequire|different:id)',
...
(
    ...
    `file_desc`    varchar(255) NOT NULL DEFAULT '' COMMENT '文件描述(noRequire)',
